...
# Learning parameters
params = {}
params['n_train_iterations']      = 10000
...
params['eps_max']                 = 0.5
params['eps_min']                 = 0.1
params['print_interval']          = 25

params['keep_prob_begin'] = 0.8
...
class EnvLog:
    ...
    def add_after_sample(self, loss):
    ...
    def save(self, f):
        pl.dump((self.duration, self.reward, self.loss), f)

    def load(self, f):
        self.duration, self.reward, self.loss = pl.load(f)
...
class DropoutModel(Model):
    def __init__(self, fc_sizes, keep_holder, input_shape=None, load_from=None):
        conv_arg = (input_shape[0], [5, 5], [32, 32], [3, 3], load_from)
        ...
        self.conv_part = ConvolutionPart(*conv_arg)
        self.input_shape = input_shape
        self.input_shape[0] = None

    def add_to_graph(self, input_tensor):
        i2 = self.conv_part.add_to_graph(input_tensor)
        interm = self.fc1.add_to_graph(i2)
    ...
    def save_weights(self, file_name, sess):
        ...
        d4 = self.conv_part.saveable_weights_dict(f, sess)
        np.savez_compressed(f, **{**d1, **d2, **d3, **d4})
