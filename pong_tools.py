...
def get_all_coords2(I):
    ...
    ball_y = np.argmax(np.sum(ball_slice, axis=1))
    # return float(opponent_y - self_y), float(ball_x - self_x), float(ball_y - self_y)
    return self_y, opponent_y, ball_x, ball_y
...
def prepro_absnormed(I_list):
    features = []
    for I in I_list:
        sy, opy, bx, by = get_all_coords2(I)
        features.append(sy / 40. - 1.)
        features.append(opy / 40. - 1.)
        features.append(bx / 30. - 1.)
        features.append(by / 40. - 1.)
